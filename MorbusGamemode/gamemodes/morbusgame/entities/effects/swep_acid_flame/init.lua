	...
	if data:GetEntity():IsValid() then
		...
		for i = 0, 1 do
			...
			if ( FlameParticle ) then
				FlameParticle:SetVelocity( (( HitPos - StartPos ):GetNormal() * math.random( 1720, 1820 )) + ( VectorRand() * math.random( 55, 65 )) )
				FlameParticle:SetLifeTime( 0 )
				FlameParticle:SetDieTime( 0.55 )
				FlameParticle:SetStartAlpha( math.random( 92, 132 ) )
				FlameParticle:SetEndAlpha( 0 )
				FlameParticle:SetStartSize( math.random( 2, 4 ) )
				FlameParticle:SetEndSize( math.random( 65, 145 ) )
				FlameParticle:SetColor( 215, 255, 5, 255 )
