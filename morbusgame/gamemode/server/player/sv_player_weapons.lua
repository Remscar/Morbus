...
function WEPS.DropNotifiedWeapon(ply, wep)
	if ValidEntity(ply) and ValidEntity(wep) then
		if wep.AllowDrop == false then -- Hack to tell the weapon it's about to be dropped and should do what it must right now
...
local function ForceWeaponSwitch(ply, cmd, args)
	...
	if ValidEntity(wep) then
		if wep.SetClip2 then -- Weapons apparently not guaranteed to have this
			wep:SetClip2(1)
		end
...
local function DropActiveAmmo(ply)
	...
	if amt < 1 or amt <= (wep.Primary.ClipSize * 0.25) then return end
