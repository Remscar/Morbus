...
function GM:PlayerDeathThink(ply)
	...
	if ply:GetMoveType() < MOVETYPE_NOCLIP and ply:GetMoveType() > 0 or ply:GetMoveType() == MOVETYPE_LADDER then -- when spectating a ragdoll after death
		ply:Spectate(OBS_MODE_ROAMING)
	end
	if ply:GetObserverMode() != OBS_MODE_ROAMING then -- when speccing a player
...
function GM:PlayerDeath(victim, weapon, killer)
	...
	if (GetRoundState() != ROUND_ACTIVE) then
		...
		return -- Don't bother with this shit if we aren't playing
	end
	if !victim:IsAlien() && victim.BroodInfect then -- Infection timer
	...
	if not killer:IsPlayer() then -- The played died from UNKNOWN causes (Falling, crushed by map)
		LogDeath(victim,"Unknown")
		Death_Unknown(victim,weapon,killer)
	elseif killer:GetActiveWeapon() == NULL or weapon == victim then -- The player killed themselves
		LogDeath(victim,"Suicide")
		Death_Suicide(victim,weapon,killer)
	elseif killer:GetActiveWeapon():GetClass() == "weapon_mor_swarm" && !victim:IsAlien() then -- The player is infected by a Swarm Alien
		LogInfect(killer,victim,"Swarm")
		Round_Swarm_Infects = Round_Swarm_Infects + 1
		Death_Swarm_infect(victim,weapon,killer)
	elseif victim:IsBrood() then -- The player was a Brood Alien & died
		LogKill(victim,killer,"Brood Killed")
		Round_Brood_Kills = Round_Brood_Kills + 1
		Death_Brood(victim,weapon,killer)
	elseif victim:IsSwarm() then -- The player was a Swarm Alien & died
		LogKill(victim,killer,"Swarm Killed")
		Round_Swarm_Kills = Round_Swarm_Kills + 1
		Death_Swarm(victim,weapon,killer)
	elseif killer:GetActiveWeapon():GetClass() ==  "weapon_mor_brood" && !victim:IsAlien() then -- The player was infected by a Brood Alien
		...
		if (victim:IsAlien() == killer:IsAlien()) then -- The player was killed by their team member
			LogRDM(killer,victim,"RDM")
			Round_RDMs = Round_RDMs + 1
		else -- The player is an alien and shot by a human, or vice versa
			LogKill(victim,killer,"KILL")
		end
		Death_Normal(victim,weapon,killer) -- Normal death
	end
end
...
function Death_Normal(ply,weapon,killer)
	...
	NewAlien(ply,ROLE_SWARM) --They were shot to death by a Brood or a Human, make them a Swarm Alien now
end
