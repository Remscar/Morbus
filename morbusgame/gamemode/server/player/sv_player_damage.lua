...
function GM:PlayerTakeDamage(ent, infl, att, amount, dmginfo)
	...
	dmginfo:SetDamage(dmginfo:GetBaseDamage() / 2) -- For unknown reasons, damage is doubled by gmod. Compensate for that here.
	...
	ent.DMG = 0 -- clear
	GAMEMODE:ScalePlayerDamage(ent, ent:LastHitGroup() or HITGROUP_GENERIC, dmginfo, true) -- Perform damage scaling
...
function GM:ScalePlayerDamage(ply, hitgroup, dmginfo, real)
	if not real then return end -- There's a phantom call to this function made by gmod, ignore it.
	...
	if atkr:IsPlayer() then
		...
		if ply:GetNWBool("alienform",false) && atkr:IsAlien() then
			dmginfo:SetDamage(0)
		end
		if ply:IsAlien() && atkr:IsSwarm() then
			dmginfo:SetDamage(0)
		end
		if ply:IsAlien() && atkr:GetNWBool("alienform") then
			dmginfo:SetDamage(0)
		end
	end
