...
function PANEL:StartUpdateTimer()
	if not timer.Exists("MScoreboardUpdater") then
		timer.Create( "MScoreboardUpdater", 0.3, 0, function()
...
function PANEL:PerformLayout()
	local gy = 0 -- position groups and find their total size
	local w = math.max(ScrW() * 0.6, 640)
	local h = y_logo_off + 110 + self.ply_frame:GetCanvas():GetTall()
	local scrolling = h > ScrH() * 0.95 -- if we will have to clamp our height, enable the mouse so player can scroll
	local hw = w - 180 - 8
	...
	h = math.Clamp(h, 110 + y_logo_off, ScrH() * 0.95) -- TODO Check if defining this twice is correct behavior
...
function PANEL:UpdateScoreboard( force )
	...
	-- Put players where they belong. Groups will dump them as soon as they don't anymore.
