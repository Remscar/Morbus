...
-- Coders note: I really hate how my bots alwasys run away from me, i just want to infect them... what assholes
	...
	for k, ent in pairs(player.GetAll()) do
		if ent != ply then
			if ((ply:GetPos():Distance(ent:GetPos()) < distance && ent:Alive()) || (ent:IsAlien() && ent:Alive())) && ent:Team() == TEAM_GAME then
				-- TODO Rename these functions to something that makes sense
				A(ent)
				B(ent)
			end
		end
