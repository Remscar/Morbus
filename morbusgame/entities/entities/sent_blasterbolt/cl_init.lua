...
function ENT:Think()
	self:NextThink( 0 )
		...
		emitter:SetPos( vOffset )
			...
			local smoke = emitter:Add( "sprites/glow04_noz", vOffset + vPos )
			smoke:SetVelocity( VectorRand() * math.Rand(-45,45) )
			...
			smoke:SetColor(Color( 255, 155, 25 ))
		...
		for i = 1,10 do
			...
			smoke:SetRoll( math.Rand(-180,180) )
			smoke:SetRollDelta( math.Rand(-0.2,0.2) )
			smoke:SetColor(Color( 255, 255, 255 ))
			smoke:SetAirResistance( 10 )
			smoke:SetCollide( true )
		end
		self.Emitter:Finish()
